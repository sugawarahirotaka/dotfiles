---
name: write-readable-japanese
description: 日本語の技術文書、実験結果、研究メモ、記事、解説、比較、原因分析、考察、進捗報告、長めのCodex回答を、パラグラフライティングと厳密な論証に基づいて作成・推敲する。整形（一文一行、脚注、コードブロック）、段落構成、論理の飛躍、読み手の負荷、視点、演出、LLMっぽい空句、冗長、見出し、Markdown、数値と不確実性の示し方を規定する。「読みやすく」「人間向けに」「自然な日本語で」「Markdownを整理して」と頼まれたとき、または短い一段落を超える日本語の技術説明・報告を書くときに使用する。短い一言、コード・ログ・生データだけの出力、文芸作品、ユーザー指定の厳密な文体やテンプレートが優先される文章には使用しない。
---

# 日本語技術文書の文章規範

日本語で技術的な原稿（書籍の章、記事、解説文）、実験報告、研究メモを作成・推敲するときは、以下の規範に従う。
ユーザーの指示、既存文書の文体、指定テンプレートを優先し、編集では事実、数値、数式、コード、リンク、引用、引用キーを変えない。

## 用途別の補助規範

- 実験結果や統計を扱うときは、作業前に[実験結果の伝え方](references/experiment-results.md)を読む。
- 数式、機構、原因、概念比較を説明するときは、作業前に[技術説明の組み立て方](references/technical-explanations.md)を読む。
- 見出し、表、箇条書き、強調、コールアウトを使う長い回答では、作業前に[Markdownの組み立て方](references/markdown-structure.md)を読む。
- 短い回答や結果報告では、最初の一、二文で答えまたは主結果を直接述べる。「結論から言うと」「以下で説明する」と予告しない。その後は根拠、境界、次の行動へ進み、同じ結論へ戻らない。
- 一から三段落で済む回答に、タイトルや見出しを付けない。
- 文体はユーザーまたは既存文書に合わせ、敬体と常体を一つの出力内で混ぜない。
- 作業予告、監査過程、質問への称賛、過度な同意を本文に混ぜない。
...
- 観測事実、そこからの解釈、未検証の仮説、提案を同じ確度で書かない。相関や同時変化だけから原因を断定しない。
- 与えられていない事実、数値、出典、因果関係を補わない。情報が足りない場合は、確認できない内容を具体的に書く。
...
- 絵文字、飾り罫線、通常の結論を囲むコールアウトを既定では使わない。
...
- 主結論に反する結果、条件依存性、失敗例を隠さない。ただし、全ての細部を主結果と同じ重さで並べず、結論の範囲を変えるものを近くに置く。
- 「差がない」「効果がない」は、それを判定できる設計と精度がある場合だけ使う。そうでない場合は「この結果では明確な差を確認できない」と範囲を限定する。

## 最終点検

- 最初の二文または冒頭の論点設定だけで、読むべき対象がわかるか。
- 各段落が前の段落から何を受け取り、次へ何を渡すか説明できるか。
- 主張の近くに根拠、機構、適用条件があるか。
- 異なる原因や確度を、滑らかな一文で混ぜていないか。
- 太字、見出し、箇条書き、コールアウトを減らしても意味が変わらないなら、減らしたか。
- LLMっぽい予告、空疎な総括、同義反復、想像上の読者との問答を削ったか。
- 数値、単位、比較基準、不確実性を原資料どおり保ったか。
